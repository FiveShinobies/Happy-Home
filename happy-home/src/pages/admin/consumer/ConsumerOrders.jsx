...
const ConsumerOrders = ({ consumer, orders, onBack, onViewOrderDetails, consumers }) => {
  const name = consumers.find(c => c.consumerId === orders.consumerId)?.name || 'Unknown Consumer';
  return (
            ...
            <tbody>
              {orders.map((order) => (
                <tr key={order.id} className={styles.tableRow}>
                  ...
                  <td>{order.serviceName} service</td>
                  <td className="fw-bold text-primary">{order.orderPrice}</td>
                  <td>
