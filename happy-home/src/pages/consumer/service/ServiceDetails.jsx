...
// Array of default fallback images
const imageData = [service1, service2, service3, service4, service5, service6];

// Default fallback image (using first image from array)
...
const base64ToImageSrc = (base64) => {
  ...
  const mimeType = base64.startsWith("iVBOR") ? "image/png" :
    base64.startsWith("/9j/") ? "image/jpeg" :
      base64.startsWith("R0lGOD") ? "image/gif" :
        base64.startsWith("UklGR") ? "image/webp" : "image/jpeg";
...
const getServiceImages = (service) => {
  if (!service || !Array.isArray(service.images)) {
    // Return random default image if no images array
  ...
  const validImages = service.images
    ...
    .filter(img => img !== null); // Remove any that failed conversion

  // If no valid images, return random default
...
const ServiceDetails = () => {
  ...
  // Get images for the service (handles base64 and null values)
                ...
                {service.reviews && service.reviews.length > 0 ? (
                  service.reviews.map((review, idx) => (
                    <div key={idx} style={styles.reviewCard}>
                      ...
                      <p className="text-muted mb-0">
                        {review.comment || review.description || review.review}
                      </p>
                    </div>
                  ))
                ) : (
